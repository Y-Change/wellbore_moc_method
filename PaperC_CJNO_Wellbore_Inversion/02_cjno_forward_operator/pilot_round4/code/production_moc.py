# -*- coding: utf-8 -*-
"""Single production Tensor MOC used for forward, ZVB memory, and train entry.

reset() restores only dynamic state. Attached differentiable parameters are
not rewritten from the constant setup arrays.
"""
from __future__ import annotations

from typing import Dict, List, Optional

import numpy as np
import torch

from ift_layer import cluster_newton
from kernel_loader import FrozenKernel
from moc_twin import setup
from stage1_bridge import F_DEFINITION, SHARED_PRIMITIVES, ms


class NewtonFailedError(RuntimeError):
    pass


def _t64(x, device=None, req=False):
    if torch.is_tensor(x):
        return x.to(dtype=torch.float64, device=device)
    return torch.tensor(x, dtype=torch.float64, device=device, requires_grad=req)


def exp_trap_weights_t(h: torch.Tensor):
    small = h < 1e-3
    wB_s = 0.5 - h / 6.0 + h * h / 24.0
    wA_s = 0.5 - h / 3.0 + h * h / 8.0
    E_s = 1.0 - h + 0.5 * h * h - h * h * h / 6.0
    E = torch.exp(-h)
    p1 = (1.0 - E) / h.clamp_min(1e-30)
    wB = p1 - (p1 - E) / h.clamp_min(1e-30)
    wA = p1 - wB
    return (
        torch.where(small, E_s, E),
        torch.where(small, wA_s, wA),
        torch.where(small, wB_s, wB),
    )


def branch_coefficients_t(dt, I_f, R_f, C_f, G_l, p_res, q_n, p_c_n, F_n,
                          inertia_scheme=1, storage_scheme=0, stiff_guard=1):
    dt = _t64(dt, I_f.device)
    guard = torch.zeros((), dtype=torch.float64, device=I_f.device)
    if inertia_scheme == 0:
        c1 = R_f + I_f / dt
        c0 = -I_f * q_n / dt
    else:
        h = R_f * dt / I_f.clamp_min(1e-30)
        E, wA, wB = exp_trap_weights_t(h)
        c1 = I_f / (dt * wB.clamp_min(1e-30))
        c0 = -c1 * E * q_n - (wA / wB.clamp_min(1e-30)) * F_n
    if storage_scheme == 0 and stiff_guard:
        ratio = G_l * dt / C_f.clamp_min(1e-30)
        switch = (C_f <= 0) | (ratio > 1.0)
        guard = switch.to(torch.float64).sum()
        use_exp_t = switch
    else:
        use_exp_t = torch.zeros_like(C_f, dtype=torch.bool)
        if storage_scheme == 1:
            use_exp_t = torch.ones_like(C_f, dtype=torch.bool)
    dn = C_f / dt + 0.5 * G_l
    a1_cn = 0.5 / dn.clamp_min(1e-30)
    a0_cn = ((C_f / dt - 0.5 * G_l) * p_c_n + 0.5 * q_n + G_l * p_res) / dn.clamp_min(1e-30)
    a1_alg = 1.0 / G_l.clamp_min(1e-30)
    a0_alg = p_res * torch.ones_like(q_n)
    h = G_l * dt / C_f.clamp_min(1e-30)
    E, wA, wB = exp_trap_weights_t(h)
    a1_ex = dt / C_f.clamp_min(1e-30) * wB
    a0_ex = p_res + (p_c_n - p_res) * E + dt / C_f.clamp_min(1e-30) * q_n * wA
    a1_e = torch.where(C_f <= 0, a1_alg, a1_ex)
    a0_e = torch.where(C_f <= 0, a0_alg, a0_ex)
    a1 = torch.where(use_exp_t, a1_e, a1_cn)
    a0 = torch.where(use_exp_t, a0_e, a0_cn)
    return c1, c0, a1, a0, guard, use_exp_t


def f_lookup_t(Q, Re_coef, logRe_min, dlogRe, fvals):
    Re = torch.clamp(torch.abs(Q) * Re_coef, min=1e-30)
    x = (torch.log10(Re) - logRe_min) / dlogRe
    n = fvals.numel()
    x = torch.clamp(x, 0.0, float(n - 1) - 1e-12)
    k = torch.floor(x).to(torch.int64)
    w = x - k.to(x.dtype)
    return fvals[k] * (1.0 - w) + fvals[k + 1] * w


def wellhead_Q_t(t, Q0, t_s, t_c):
    t = t if torch.is_tensor(t) else _t64(t, Q0.device)
    Q0 = Q0 if torch.is_tensor(Q0) else _t64(Q0, t.device)
    closed = t >= (t_s + t_c)
    xi = (t - t_s) / max(float(t_c), 1e-30)
    qmid = 0.5 * Q0 * (1.0 + torch.cos(torch.tensor(np.pi, dtype=torch.float64, device=Q0.device) * xi))
    return torch.where(t < t_s, Q0, torch.where(closed | (t_c <= 0), torch.zeros_like(Q0), qmid))


def foot_t(Hn, Qn, Ju, i, neigh, Cr):
    if abs(float(Cr) - 1.0) < 1e-15:
        return Hn[neigh], Qn[neigh], Ju[neigh]
    w = Cr
    return ((1.0 - w) * Hn[i] + w * Hn[neigh],
            (1.0 - w) * Qn[i] + w * Qn[neigh],
            (1.0 - w) * Ju[i] + w * Ju[neigh])


def flow_from_char_t(D, twoB, twoR):
    aD = torch.abs(D)
    disc = torch.sqrt(twoB * twoB + 2.0 * twoR * aD)
    return torch.where(twoR <= 0, D / twoB, torch.sign(D) * 2.0 * aD / (twoB + disc))


PARAM_NAMES = ("pK", "pkappa", "pIf", "pRf", "pCf", "pGl", "ppres")


class ProductionMOC:
    def __init__(self, well, num, friction_model="darcy", device="cpu",
                 frozen: Optional[FrozenKernel] = None):
        if friction_model not in ("none", "darcy", "zvb_rec"):
            raise ValueError("production path: none/darcy/zvb_rec")
        self.well = well
        self.num = num
        self.friction_model = friction_model
        self.device = torch.device(device)
        self.S = setup(well, num, friction_model, frozen=frozen)
        if self.S["fric_mode"] == 3:
            raise RuntimeError("fit-kernel / zvb_direct branch is forbidden")
        self.grid = self.S["grid"]
        self.dt = float(self.S["dt"])
        self.F_definition = dict(F_DEFINITION)
        self.shared_primitives = list(SHARED_PRIMITIVES)
        self.last_resid: List[float] = []
        self.last_guard_trace: List[dict] = []
        self.n_fail = 0
        self.newton_calls = 0
        ftab = self.S["ftab"]
        self.fvals = _t64(ftab.values, self.device)
        self.logRe_min = float(ftab.logRe_min)
        self.dlogRe = float(ftab.dlogRe)
        self.Re_coef = float(self.S["Re_coef"])
        self._attached = set()
        self._bind_static()
        self.reset()

    def _bind_static(self):
        d = self.device
        S = self.S
        for name in PARAM_NAMES:
            setattr(self, name, _t64(S[name], d))
        self.cl_z = _t64(S["cl_z"], d)
        self.seg_B = _t64(S["seg_B"], d)
        self.seg_Rcoef = _t64(S["seg_Rcoef"], d)
        self.rho_g = _t64(S["rho_g"], d)
        self.A = _t64(S["A"], d)
        self.H_scale = float(S["H_scale"])
        self.Q_scale = float(S["Q_scale"])
        self.p_scale = float(S["p_scale"])
        self.eps_q = float(S["eps_q"])
        if S["kern"] is not None:
            self.kern_E = _t64(S["kern"]["E"], d)
            self.kern_Phi = _t64(S["kern"]["Phi"], d)
            self.kern_w = _t64(S["kern"]["w"], d)
        else:
            self.kern_E = self.kern_Phi = self.kern_w = None

    def attach_param(self, name: str, tensor: torch.Tensor) -> None:
        if name not in PARAM_NAMES:
            raise ValueError(name)
        setattr(self, name, tensor)
        self._attached.add(name)

    def reset(self):
        """Reset dynamic state only. Attached parameters stay as attached."""
        S = self.S
        d = self.device
        self.H = _t64(S["H0"], d)
        self.Q = _t64(S["Q0"], d)
        self.Ju = torch.zeros_like(self.H)
        self.q = _t64(S["q0"], d)
        self.pc = _t64(S["pc0"], d)
        self.F = _t64(S["F0"], d)
        self.z = torch.zeros((S["M"], self.grid.n_nodes), dtype=torch.float64, device=d)
        self.time_n = 0
        self.n_fail = 0
        self.newton_calls = 0
        self.last_resid = []
        self.last_guard_trace = []

    def snapshot_dynamic(self) -> dict:
        return {
            "H": self.H.detach().clone(),
            "Q": self.Q.detach().clone(),
            "Ju": self.Ju.detach().clone(),
            "q": self.q.detach().clone(),
            "pc": self.pc.detach().clone(),
            "F": self.F.detach().clone(),
            "z": self.z.detach().clone(),
            "time_n": int(self.time_n),
        }

    def restore_dynamic(self, snap: dict) -> None:
        self.H = snap["H"].clone()
        self.Q = snap["Q"].clone()
        self.Ju = snap["Ju"].clone()
        self.q = snap["q"].clone()
        self.pc = snap["pc"].clone()
        self.F = snap["F"].clone()
        self.z = snap["z"].clone()
        self.time_n = int(snap["time_n"])

    def _f(self, Qv):
        if self.S["fric_mode"] == 0:
            return torch.zeros((), dtype=torch.float64, device=self.device)
        return f_lookup_t(Qv, self.Re_coef, self.logRe_min, self.dlogRe, self.fvals)

    def p_head(self):
        return self.rho_g * self.H[0]

    def _interior_segment(self, H, Q, Hn, Qn, Ju, j):
        grid = self.grid
        dt = self.dt
        o = int(grid.seg_off[j])
        N = int(grid.seg_N[j])
        if N <= 1:
            return
        Cr = float(grid.seg_Cr[j])
        B = self.seg_B[j]
        Rc = self.seg_Rcoef[j]
        idx = torch.arange(o + 1, o + N, device=self.device)
        if abs(Cr - 1.0) < 1e-15:
            HA, QA, JA = Hn[idx - 1], Qn[idx - 1], Ju[idx - 1]
            HB, QB, JB = Hn[idx + 1], Qn[idx + 1], Ju[idx + 1]
        else:
            w = Cr
            HA = (1.0 - w) * Hn[idx] + w * Hn[idx - 1]
            QA = (1.0 - w) * Qn[idx] + w * Qn[idx - 1]
            JA = (1.0 - w) * Ju[idx] + w * Ju[idx - 1]
            HB = (1.0 - w) * Hn[idx] + w * Hn[idx + 1]
            QB = (1.0 - w) * Qn[idx] + w * Qn[idx + 1]
            JB = (1.0 - w) * Ju[idx] + w * Ju[idx + 1]
        RA = self._f(QA) * Rc
        RB = self._f(QB) * Rc
        JuA = B * dt * self.A * JA
        JuB = B * dt * self.A * JB
        if self.S["trap"]:
            RP = self._f(Qn[idx]) * Rc
            CP = HA + B * QA - 0.5 * RA * QA * torch.abs(QA) - JuA
            CM = HB - B * QB + 0.5 * RB * QB * torch.abs(QB) + JuB
            QP = flow_from_char_t(CP - CM, 2.0 * B, 2.0 * RP)
            if self.S["fric_mode"] != 0:
                RP = self._f(QP) * Rc
                QP = flow_from_char_t(CP - CM, 2.0 * B, 2.0 * RP)
            HP = CP - B * QP - 0.5 * RP * QP * torch.abs(QP)
        else:
            CP = HA + B * QA - RA * QA * torch.abs(QA) - JuA
            CM = HB - B * QB + RB * QB * torch.abs(QB) + JuB
            QP = (CP - CM) / (2.0 * B)
            HP = 0.5 * (CP + CM)
        H[idx] = HP
        Q[idx] = QP

    def _update_memory(self, Q_new):
        if self.S["fric_mode"] != 2:
            return
        V_prev = self.Q / self.A
        V = Q_new / self.A
        dV = V - V_prev
        z = self.z.clone()
        Ju = self.Ju.clone()
        grid = self.grid
        for j in range(grid.nseg):
            o = int(grid.seg_off[j])
            N = int(grid.seg_N[j])
            sl = slice(o, o + N + 1)
            z[:, sl] = self.kern_E[j, :, None] * z[:, sl] + self.kern_Phi[j, :, None] * dV[sl]
            Ju[sl] = self.kern_w[j] * z[:, sl].sum(dim=0)
        self.z = z
        self.Ju = Ju

    def step(self, require_ok: bool = True) -> dict:
        grid = self.grid
        dt = self.dt
        nseg = grid.nseg
        Hn, Qn, Ju = self.H, self.Q, self.Ju
        H = self.H.clone()
        Q = self.Q.clone()
        t = (self.time_n + 1) * dt
        for j in range(nseg):
            self._interior_segment(H, Q, Hn, Qn, Ju, j)
        Cr = float(grid.seg_Cr[0])
        B = self.seg_B[0]
        Rc = self.seg_Rcoef[0]
        i = int(grid.seg_off[0])
        HB, QB, JB = foot_t(Hn, Qn, Ju, i, i + 1, Cr)
        RB = self._f(QB) * Rc
        JuB = B * dt * self.A * JB
        QP = wellhead_Q_t(_t64(t, self.device), _t64(self.well.Q0, self.device),
                          self.well.t_s, self.well.t_c)
        if self.S["trap"]:
            RP = self._f(QP) * Rc
            CM = HB - B * QB + 0.5 * RB * QB * torch.abs(QB) + JuB
            HP = CM + B * QP + 0.5 * RP * QP * torch.abs(QP)
        else:
            CM = HB - B * QB + RB * QB * torch.abs(QB) + JuB
            HP = CM + B * QP
        H[i] = HP
        Q[i] = QP
        j = nseg - 1
        o = int(grid.seg_off[j])
        N = int(grid.seg_N[j])
        Cr = float(grid.seg_Cr[j])
        B = self.seg_B[j]
        Rc = self.seg_Rcoef[j]
        i = o + N
        HA, QA, JA = foot_t(Hn, Qn, Ju, i, i - 1, Cr)
        RA = self._f(QA) * Rc
        JuA = B * dt * self.A * JA
        if self.S["trap"]:
            CP = HA + B * QA - 0.5 * RA * QA * torch.abs(QA) - JuA
        else:
            CP = HA + B * QA - RA * QA * torch.abs(QA) - JuA
        H[i] = CP
        Q[i] = torch.zeros((), dtype=torch.float64, device=self.device)
        q_new = self.q.clone()
        pc_new = self.pc.clone()
        F_new = self.F.clone()
        resid_step = 0.0
        guard_step = 0.0
        n_switch = 0
        for jc in range(self.S["ncl"]):
            iL = grid.cluster_left_index(jc)
            iR = grid.cluster_right_index(jc)
            CrL = float(grid.seg_Cr[jc])
            BL = self.seg_B[jc]
            RcL = self.seg_Rcoef[jc]
            HA, QA, JA = foot_t(Hn, Qn, Ju, iL, iL - 1, CrL)
            RA = self._f(QA) * RcL
            JuA = BL * dt * self.A * JA
            CrR = float(grid.seg_Cr[jc + 1])
            BR = self.seg_B[jc + 1]
            RcR = self.seg_Rcoef[jc + 1]
            HB, QB, JB = foot_t(Hn, Qn, Ju, iR, iR + 1, CrR)
            RB = self._f(QB) * RcR
            JuB = BR * dt * self.A * JB
            if self.S["trap"]:
                RPL = self._f(Qn[iL]) * RcL
                RPR = self._f(Qn[iR]) * RcR
                CP = HA + BL * QA - 0.5 * RA * QA * torch.abs(QA) - JuA
                CM = HB - BR * QB + 0.5 * RB * QB * torch.abs(QB) + JuB
            else:
                RPL = torch.zeros((), dtype=torch.float64, device=self.device)
                RPR = torch.zeros((), dtype=torch.float64, device=self.device)
                CP = HA + BL * QA - RA * QA * torch.abs(QA) - JuA
                CM = HB - BR * QB + RB * QB * torch.abs(QB) + JuB
            m0, m1 = int(self.S["perf_off"][jc]), int(self.S["perf_off"][jc + 1])
            c1, c0, a1, a0, guard, use_exp = branch_coefficients_t(
                dt, self.pIf[m0:m1], self.pRf[m0:m1], self.pCf[m0:m1], self.pGl[m0:m1],
                self.ppres[m0:m1], self.q[m0:m1], self.pc[m0:m1], self.F[m0:m1],
                self.S["inertia"], self.S["storage"], self.S["stiff_guard"],
            )
            guard_step += float(guard.detach())
            n_switch += int(use_exp.to(torch.int64).sum().detach())
            self.newton_calls += 1
            Hj, Qm, Qp, qj, stat = cluster_newton(
                CP, CM, BL, BR, RPL, RPR, self.cl_z[jc], self.rho_g,
                self.pK[m0:m1], self.pkappa[m0:m1], self.eps_q,
                c1, c0, a1, a0, Hn[iL], self.q[m0:m1],
                self.H_scale, self.Q_scale, self.p_scale,
                self.num.tol_dimless, self.num.max_iter, self.num.max_inner,
            )
            ok = bool(float(stat[1].detach()) > 0.5)
            rinf = float(stat[2].detach())
            tikh = bool(float(stat[4].detach()) > 0.5)
            resid_step = max(resid_step, rinf)
            if not ok:
                self.n_fail += 1
                if require_ok:
                    raise NewtonFailedError(
                        f"real Newton failure at step={self.time_n+1} cluster={jc} rinf={rinf}"
                    )
            H[iL] = Hj
            H[iR] = Hj
            Q[iL] = Qm
            Q[iR] = Qp
            q_new[m0:m1] = qj
            pc_new[m0:m1] = a0 + a1 * qj
            F_new[m0:m1] = c1 * qj + c0
            self.last_guard_trace.append({
                "step": self.time_n + 1,
                "cluster": jc,
                "guard_sum": float(guard.detach()),
                "n_storage_switch": int(use_exp.to(torch.int64).sum().detach()),
                "tikhonov": tikh,
                "ok": ok,
                "rinf": rinf,
                "nit": float(stat[0].detach()),
            })
        self._update_memory(Q)
        self.H, self.Q = H, Q
        self.q, self.pc, self.F = q_new, pc_new, F_new
        self.time_n += 1
        self.last_resid.append(resid_step)
        return {"p_head": self.p_head(), "ok": True, "resid": resid_step,
                "guard_sum": guard_step, "n_storage_switch": n_switch}

    def unroll(self, n_steps: int, require_ok: bool = True,
               record_state: bool = False) -> torch.Tensor:
        ps = [self.p_head()]
        rec = None
        if record_state:
            rec = {
                "q": [self.q.detach().cpu().numpy().copy()],
                "pc": [self.pc.detach().cpu().numpy().copy()],
                "F": [self.F.detach().cpu().numpy().copy()],
            }
        for _ in range(int(n_steps)):
            self.step(require_ok=require_ok)
            ps.append(self.p_head())
            if rec is not None:
                rec["q"].append(self.q.detach().cpu().numpy().copy())
                rec["pc"].append(self.pc.detach().cpu().numpy().copy())
                rec["F"].append(self.F.detach().cpu().numpy().copy())
        out = torch.stack(ps)
        if rec is not None:
            self._last_archive = rec
        return out

    def export_numpy_state(self) -> dict:
        return {
            "H": self.H.detach().cpu().numpy(),
            "Q": self.Q.detach().cpu().numpy(),
            "Ju": self.Ju.detach().cpu().numpy(),
            "z": self.z.detach().cpu().numpy(),
            "q": self.q.detach().cpu().numpy(),
            "pc": self.pc.detach().cpu().numpy(),
            "F": self.F.detach().cpu().numpy(),
            "perf_off": self.S["perf_off"].copy(),
            "time_n": int(self.time_n),
            "full_state_exported": True,
            "n_clusters": int(self.S["ncl"]),
            "ntot": int(self.S["ntot"]),
        }


class TrainEntry:
    """Optimizer-facing step on the same ProductionMOC. Failures do not advance state."""

    def __init__(self, roll: ProductionMOC, optimizer, scheduler=None):
        self.roll = roll
        self.opt = optimizer
        self.sched = scheduler
        self.global_step = 0

    def differentiable_params(self):
        seen = []
        if self.opt is not None:
            for g in self.opt.param_groups:
                seen.extend(g["params"])
        for name in PARAM_NAMES:
            t = getattr(self.roll, name)
            if torch.is_tensor(t) and t.requires_grad and t.is_leaf:
                if t not in seen:
                    seen.append(t)
        return seen

    def step_loss(self, n_steps: int, loss_fn, require_ok: bool = True):
        params_before = {n: getattr(self.roll, n).detach().clone()
                         for n in PARAM_NAMES if n in self.roll._attached}
        opt_sd_before = self.opt.state_dict() if self.opt is not None else None
        sched_before = self.sched.state_dict() if self.sched is not None else None
        gstep_before = int(self.global_step)
        self.roll.reset()
        for n, t in params_before.items():
            if not torch.equal(getattr(self.roll, n).detach(), t):
                raise RuntimeError(f"reset() restored attached parameter {n}")
        dyn = self.roll.snapshot_dynamic()
        try:
            pred = self.roll.unroll(n_steps, require_ok=require_ok)
            loss = loss_fn(pred)
            if not torch.isfinite(loss):
                self.roll.restore_dynamic(dyn)
                return {"ok": False, "reason": "nonfinite_loss", "advanced": False, "loss": None}
            if self.opt is not None:
                self.opt.zero_grad(set_to_none=True)
            loss.backward()
            params = self.differentiable_params()
            if not params:
                self.roll.restore_dynamic(dyn)
                return {"ok": False, "reason": "empty_grad_params", "advanced": False, "loss": float(loss.detach())}
            if any(p.grad is None for p in params) or any(not torch.isfinite(p.grad).all() for p in params if p.grad is not None):
                if self.opt is not None:
                    self.opt.zero_grad(set_to_none=True)
                self.roll.restore_dynamic(dyn)
                return {"ok": False, "reason": "nonfinite_or_missing_grad", "advanced": False,
                        "loss": float(loss.detach())}
            if self.opt is not None:
                self.opt.step()
            if self.sched is not None:
                self.sched.step()
            self.global_step += 1
            return {"ok": True, "reason": "stepped", "advanced": True, "loss": float(loss.detach()),
                    "pred": pred}
        except NewtonFailedError as exc:
            self.roll.restore_dynamic(dyn)
            if self.opt is not None:
                self.opt.zero_grad(set_to_none=True)
            return {
                "ok": False, "reason": "newton_fail", "advanced": False, "error": str(exc),
                "global_step": self.global_step, "global_step_before": gstep_before,
                "opt_unchanged": True, "sched_unchanged": True,
                "opt_before_keys": list(opt_sd_before.keys()) if opt_sd_before else [],
                "sched_before": sched_before,
            }
