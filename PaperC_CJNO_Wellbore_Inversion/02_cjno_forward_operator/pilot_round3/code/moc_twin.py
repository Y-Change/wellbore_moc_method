# -*- coding: utf-8 -*-
"""Python discrete twin of Stage-1 MOC (none / Darcy / optional ZVB-rec).

Exports true per-perf q, pc, F and left/right traces. Does not change Stage-1
njit defaults. Cr<1 uses the same foot interpolation as moc_solver; travel time
shorter than dt is not clipped to one step.
"""
...
from stage1_bridge import (
    F_DEFINITION, branch_coefficients, build_grid, flow_from_characteristic, ms,
    solve_cluster_node,
)
...
def _setup(well, num, friction_model: str):
    A = well.A
    g = well.g
    rho_g = well.rho * g
    ...
    ftab = ms.FrictionTable.build(well.roughness / well.D) if hasattr(ms, "FrictionTable") else None
    if ftab is None:
        from friction import FrictionTable
...
def run_twin(well, num, friction_model: str = "darcy", record_every: int = 1) -> TwinResult:
    if friction_model not in ("none", "darcy"):
        raise ValueError(f"twin supports none/darcy in this file; got {friction_model}")
    S = _setup(well, num, friction_model)
    grid = S["grid"]
    dt = S["dt"]
    n_steps = S["n_steps"]
    H = S["H0"].copy()
    ...
    Q_head[0] = Q[0]
    ncl = S["ncl"]
    rec_n = n_steps // max(record_every, 1) + 1
    node_H = np.zeros((rec_n, ncl))
    node_Qm = np.zeros((rec_n, ncl))
    node_Qp = np.zeros((rec_n, ncl))
    node_sq = np.zeros((rec_n, ncl))
    node_pc = np.zeros((rec_n, ncl))
    q_hist = [np.zeros((rec_n, well.clusters[j].nperf)) for j in range(ncl)]
    pc_hist = [np.zeros((rec_n, well.clusters[j].nperf)) for j in range(ncl)]
    F_hist = [np.zeros((rec_n, well.clusters[j].nperf)) for j in range(ncl)]
    ...
    def _record(k):
        for jc in range(ncl):
            ...
            node_Qm[k, jc] = Q[iL]
            node_Qp[k, jc] = Q[iR]
            node_sq[k, jc] = q[m0:m1].sum()
            node_pc[k, jc] = float(np.mean(pc[m0:m1])) if m1 > m0 else 0.0
            q_hist[jc][k] = q[m0:m1]
            pc_hist[jc][k] = pc[m0:m1]
            F_hist[jc][k] = Fb[m0:m1]
    ...
    for n in range(1, n_steps + 1):
        ...
        for j in range(nseg):
            ...
            for i in range(o + 1, o + N):
                HA, QA, JA = _foot(Hn, Qn, Ju, i, i - 1, Cr)
                HB, QB, JB = _foot(Hn, Qn, Ju, i, i + 1, Cr)
                RA = f_of(QA) * Rc
                RB = f_of(QB) * Rc
                JuA = B * dt * A * JA
                JuB = B * dt * A * JB
        ...
        Q_head[n] = Q[0]
        if n % record_every == 0:
            _record(n // record_every)
    return TwinResult(
        t=t, p_head=p_head, Q_head=Q_head, H=H, Q=Q,
        q=q_hist, pc=pc_hist, F=F_hist,
        node_H=node_H, node_Qm=node_Qm, node_Qp=node_Qp, node_sq=node_sq, node_pc=node_pc,
        n_fail=n_fail, resid_max=resid_max, dt=dt, grid_summary=grid.summary(),
        friction_model=friction_model, friction_time_scheme=num.friction_time_scheme,
        notes=notes,
    )


def long_window_seconds(well) -> float:
    """Cover valve, first-cluster round trip, and two 4L/a periods."""
    period = 4.0 * well.L / well.a
