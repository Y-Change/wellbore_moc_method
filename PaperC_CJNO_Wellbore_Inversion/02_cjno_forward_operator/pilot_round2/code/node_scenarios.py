# -*- coding: utf-8 -*-
"""Packs that actually realize stiff / reverse / near-zero / hetero / Cr branches."""
from __future__ import annotations

import numpy as np

from node_coeffs import collect_branch_coeffs, pack_from_physical
...
def scenario(name: str) -> dict:
    ...
    if name == "nominal":
        ...
        p = _base_phys(4)
        # G_l dt / C_f > 1 so CN would oscillate and guard must fire
        ...
        p = _base_phys(4)
        # shallow node, low H so p_w << p_c → backflow
        M = 4
        s = _pack(p, dt, np.full(M, -0.002), np.full(M, 4.05e7), np.full(M, -1e5),
        ...
        # nearly dependent perfs + extreme scale contrast to lift cond(J)
        p = _base_phys(2)
        ...
        s = _pack(p, dt, np.array([0.01, 1e-12]), np.array([4e7, 4e7]), np.zeros(M),
                  ...
                  H_scale=4000.0, Q_scale=0.05, p_scale=1.0)  # p_scale mismatch inflates cond
