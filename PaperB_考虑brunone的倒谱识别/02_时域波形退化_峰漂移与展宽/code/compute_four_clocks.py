...
INPUT_DIR = os.path.join(_root, 'PaperB_考虑brunone的倒谱识别', '01_非定常摩阻正演与基准', 'data', 'timeseries')
...
def load_case_data(n: int, D: int, k: float):
    if n == 1:
        csv_path = os.path.join(INPUT_DIR, f'n1_k{k}.csv.gz')
    else:
        k_str = '0' if float(k) == 0.0 else str(k)
        csv_path = os.path.join(INPUT_DIR, f'n{n}_D{D}_k{k_str}.csv.gz')
