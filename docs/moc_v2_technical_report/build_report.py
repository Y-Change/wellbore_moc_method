...
"""
build_report.py
Generates and writes the fully restructured MOC_V2_Physics_Upgrade_Report.md
adhering strictly to user requirements and implementation_plan.md.
"""
import os

SCRIPT_DIR = os.path.dirname(os.path.abspath(__file__))
REPORT_PATH = os.path.join(SCRIPT_DIR, "MOC_V2_Physics_Upgrade_Report.md")

REPORT_CONTENT = r"""# MOC_V2 水力压裂井筒水击波物理内核升级技术报告
...
print(f"Successfully generated and wrote {REPORT_PATH}")

# --- Automated Integrity & Completeness Self-Verification ---
with open(REPORT_PATH, "r", encoding="utf-8") as f:
    generated_content = f.read()

assert len(generated_content) > 50000, f"Report content too short: {len(generated_content)} bytes"

# LaTeX balance verification
...
required_keywords = [
    ...
    r"dH - B dQ - a J dt = 0",
    r"B = \frac{a}{g A}",
    r"Cr = \frac{a \Delta t}{\Delta x}",
    r"R = \frac{f \Delta x}{2 g D A^2}",
    r"C_P = H_{i-1}^n + B Q_{i-1}^n - R Q_{i-1}^n |Q_{i-1}^n| - a \Delta t J_{u, i-1}^n",
    r"C_M = H_{i+1}^n - B Q_{i+1}^n + R Q_{i+1}^n |Q_{i+1}^n| + a \Delta t J_{u, i+1}^n",
    r"H_i^{n+1} = \frac{C_P + C_M}{2}",
    r"Q_i^{n+1} = \frac{C_P - C_M}{2 B}",
    r"C_P^{\text{vel}} = \frac{C_P}{B A}",
    r"C_M^{\text{vel}} = \frac{C_M}{B A}",
    r"J_u = \frac{k_B}{2 g} \left( \frac{\partial V}{\partial t} + a \cdot \mathrm{sign}(V) \left| \frac{\partial V}{\partial x} \right| \right)",
    "V_{smooth} = 0.05",
    # 2.3 Geology, compliance, leakoff
    "2.3.1 顺应性双重物理机制解耦（岩石骨架弹性形变 vs 流体声容）",
    "2.3.2 三大经典裂缝力学构型的弹性变形柔度推导与量纲修正（Penny, PKN, KGD）",
    "2.3.3 量级文献深度对齐论证（Luo et al. 2023, Valko 1995）",
    "2.3.4 滤失模型与参数（$k_{leak}, H_{ext}$）油藏物理对齐、Carter 理论与双轨制取值论证",
    r"C_{p, rock}^{Penny} = \frac{dV_f}{dp} = \frac{8 (1 - \nu^2)}{3 E} R^3 = \frac{4 (1 - \nu)}{3 G_{shear}} R^3",
    r"C_{p, rock}^{PKN} = \frac{dV_f}{dp} = \frac{\pi (1 - \nu^2) h_f^2 L_f}{E}",
    r"C_{p, rock}^{KGD} = \frac{dV_f}{dp} = \frac{\pi (1 - \nu^2) h_f L_f^2}{E}",
    # 2.6 Steady state, field pressure, parameter table
    "2.6.1 基态流场空间解析积分与动量守恒自洽场",
    "2.6.2 井口基准水头 $H_{wellhead, 0}$ 的物理实质与超静水范式论证",
    "2.6.3 现场工程绝对参数与 MOC_V2 超静水相对水头参数多维度高密度对照表",
    r"H_{wellhead} \in [200, 600]",
    r"H_{ext} \in [50, 200]",
    r"k_{leak} \in [10^{-5}, 10^{-3}]",
    r"40.0\sim 95.0",
    r"30.0\sim 65.0",
    r"1.5\sim 4.5",
    r"0.5, 3.0",
    "Luo et al. (2023, *SPE J*)",
    "Cramer (2019, *SPE HFTC*)",
    "Economides & Nolte (2000)",
    "Carter (1957)",
    "Zoback (2010)",
    "Howard & Fast (1970)",
    "Valko (1995)",
    # 2.6.4 5 Canonical Fracture Types and physical coupling
    "2.6.4 水平井多簇压裂 5 大典型裂缝类型划分、物理特征与现场参数组合矩阵",
    "Type I: 优势发育主进液簇",
    "Type II: 均衡/正常发育簇",
    "Type III: 受抑/欠发育弱进液簇",
    "Type IV: 砂堵闭合/未起裂死簇",
    "Type V: 沟通天然断层/强微裂缝簇",
    r"C_{f, j} = C_{f, base} \cdot \left( \frac{w_j}{w_{avg}} \right)^{\alpha_{cf}}",
    r"k_{leak, j} = k_{leak, base} \cdot \left( \frac{w_j}{w_{avg}} \right)^{\beta_{leak}}",
    r"K_{p, j} = \frac{1}{2 g C_{d, j}^2 A_{p, j}^2}",
    "sample_preset_scenario",
    # 2.7 Newton solver
    "2.7 非线性耦合方程唯一物理实根定理与牛顿迭代收敛性",
    r"H_{well, j}(q_p) = H_{moc0} - \frac{B}{2} q_p",
    r"F(q_p) = q_p - \left[ \frac{C_f}{\Delta t} \left( H_{frac, j}(q_p) - H_{frac, j}^{old} \right) + q_{leak, j}(H_{frac, j}(q_p)) \right] = 0",
    r"F'(q_p) \ge 1.0 > 0",
]

missing = [kw for kw in required_keywords if kw not in generated_content]
if missing:
    raise AssertionError(f"Validation failed: missing {len(missing)} required keywords: {missing}")

print(f"Validation PASSED: All {len(required_keywords)} structural, mathematical, and physical keywords verified.")
print(f"LaTeX syntax: $$ pairs = {c2 // 2} (balanced), single $ pairs = {c1 // 2} (balanced).")
