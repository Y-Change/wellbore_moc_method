...
def plot_case(ax, sp, title):
    raw_dir = output_path(SERIES_DECAY_REGRESSION, 'raw_data', '')
    npz_path = os.path.join(raw_dir, f'steady_x1_1000_sp_{sp}_n_8.npz')
    ...
    ax.plot(d_plot, p_plot, 'k-', lw=1.2, label='2D Cepstrum Profile')
    ...
    for i, xf in enumerate(x_f_aligned):
        ...
        ax.plot(peak_d, peak_val, 'o', color=colors[i], markersize=5, 
...
def main():
    ...
    plot_case(axes[0], 80, '2D Cepstrum Global Zoom (steady, x1=1000, S=80, n=8)')
    plot_case(axes[1], 100, '2D Cepstrum Global Zoom (steady, x1=1000, S=100, n=8)')
    ...
    save_path = os.path.join(out_dir, 'steady_cepstrum_x1_1000_s80_vs_s100_zoom.png')
    save_figure(fig, save_path)
    
    art_dir = r"C:\Users\Change\.gemini\antigravity\brain\4850e2b5-0e72-4d7e-8d5e-d9a05614c45c"
    art_path = os.path.join(art_dir, 'steady_cepstrum_x1_1000_s80_vs_s100_zoom.png')
